...
const VALID_VPC_EGRESS_TYPES = ['VPC_CONNECTOR_EGRESS_SETTINGS_UNSPECIFIED', 'PRIVATE_RANGES_ONLY', 'ALL_TRAFFIC'];
const VALID_INGRESS_TYPES = ['INGRESS_SETTINGS_UNSPECIFIED', 'ALLOW_ALL', 'ALLOW_INTERNAL_ONLY', 'ALLOW_INTERNAL_AND_GCLB'];
...
const validateVpcEgressProperty = (funcObject, functionName) => {
  if (funcObject.egress && typeof funcObject.egress === 'string') {
    if (!VALID_VPC_EGRESS_TYPES.includes(funcObject.egress)) {
      const errorMessage = [
...
const validateVpcIngressProperty = (funcObject, functionName) => {
  if (funcObject.ingress && typeof funcObject.ingress === 'string') {
    if (!VALID_INGRESS_TYPES.includes(funcObject.ingress)) {
      const errorMessage = [
